...
export class CheckoutPage implements OnInit {
  ...
  method_shipping = "wp-json/wc/v3/shipping_methods";  
    ...
    private loadingController: LoadingController) { 
      ...
      this.storage.get("userLoginInfo").then((userLoginInfo) => {
        ...
        this.httpClient.get(this.linkService.getAPILink() + this.customers_link +'/' + id + '?consumer_key='+ this.linkService.getConsumerKey() + '&consumer_secret='+this.linkService.getConsumerSecret()).subscribe((data: any)=>{
          ...
          this.newOrder = data;
          console.log('Datos cunsumers: ',this.newOrder);
        },
        (err)=>{
          this.dismissLoader(0);
          console.log(err);
      ...
      this.httpClient.get(this.linkService.getAPILink() + this.method_shipping + '/' + '?consumer_key='+ this.linkService.getConsumerKey() + '&consumer_secret='+this.linkService.getConsumerSecret()).subscribe((data: any)=>{
        // this.dismissLoader(0);
        this.shipping = data;
        console.log('Metodos de envio:',this.shipping);
      },
      (err)=>{
        // this.dismissLoader(0);
        console.log(err);
